...
function fixSitemap(filePath) {
  try {
    ...
    // Fix 3: Fix hreflang attributes that have extra quotes
    content = content.replace(/hreflang="([^"]+)"/g, 'hreflang="$1"');
    
    // Fix 4: Fix href attributes that have extra spaces before closing
    content = content.replace(/href="([^"]+?)"\s+\/\s+\/>/g, 'href="$1" />');
    
    // Fix 5: Fix self-closing xhtml:link tags that have extra spaces
    content = content.replace(/(<xhtml:link[^>]+)\/\s+\/>/g, '$1 />');
    
    // Fix 6: Fix xhtml:link tags that are not properly self-closing
