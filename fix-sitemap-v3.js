...
function fixSitemap(filePath) {
  try {
    ...
    // Fix 3: Fix rel=alternate attributes (missing quotes around rel)
    content = content.replace(/rel=alternate(\s|>)/g, 'rel="alternate"$1');
    
    // Fix 4: Fix href attributes that are missing closing quotes
    content = content.replace(/href="([^"]+?)\s/g, 'href="$1" ');
    
    // Fix 5: Remove trailing quotes from URLs in href attributes
    content = content.replace(/(href="[^"]+)"/g, '$1');
    
    // Fix 6: Fix self-closing xhtml:link tags that are missing the /
    content = content.replace(/(<xhtml:link[^>]+)>/g, '$1 />');
    
    // Fix 7: Ensure the file has proper closing tag
